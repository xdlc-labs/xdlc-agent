...
- **The README leads with a real Fix pull request**, [#51](https://github.com/xdlc-labs/xdlc-agent/pull/51) on this repository, instead of the demo GIF. The GIF fixes a toy `a - b` in a throwaway repo, which demonstrates the loop and undersells the product; a pull request with a failing-run link, the agent's verdict and a cost is the artifact a reader already knows how to judge. The GIF moved down to the install section with its scope stated, and `docs/assets/pr.sh` regenerates the screenshot
